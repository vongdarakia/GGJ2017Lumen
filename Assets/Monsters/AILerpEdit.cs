...
public class AILerpEdit : MonoBehaviour {
	public float speedUpRate = 10F;
	public float lastSpeedUp = -9999;
	public float speedUpAmount = 10;
	...
	{
		if (Time.time - lastSpeedUp >= speedUpRate) {
			minSpeed += speedUpAmount;
	...
	protected virtual void Update () {
		...
		Player player =  enemy.GetClosest();
		if (!player)
			return;
		if (!enemy.GetClosestPlayerBySight() && !enemy.GetClosestPlayerBySound())
			speed = minSpeed;
		else
			speed = minSpeed * 2;
		print ("AY");
