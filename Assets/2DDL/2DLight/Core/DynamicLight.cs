namespace DynamicLight2D
{
	...
	using UnityEngine.Events;
		...
		public class DynamicLight : MonoBehaviour {
										...
										else
										{
											...
											if(notifyReachedEventUsed && this.collider.type == CasterCollider.CasterType.PolygonCollider2d || this.collider.type == CasterCollider.CasterType.BoxCollider2d)
											{
													...
													foreach(Vector2 objPoint in this.collider.points)
													{
														if(360 != Mathf.RoundToInt(RangeLightAngle)){ 
															if (Vector3.Angle(lightObjectTM.InverseTransformPoint(pos), Vector3.up) < RangeLightAngle*.5f) {	// Light angle restriction
																if(((Vector2)pos - objPoint).sqrMagnitude < 500f)
				...
				/// <summary>
				/// Persist Callback all caster within sight of current 2DLight. "Require notifyGameObjectsReached = true"
				/// </summary>
																	...
																	if(notifyGameObjectsReached )
																	{
																		if(360 != Mathf.RoundToInt(RangeAngle)){ 
																			if (Vector3.Angle(transform.InverseTransformPoint(hitp), Vector3.up) < RangeAngle*.5f) {	// Light angle restriction
																				if(_ray.collider.gameObject.transform.parent){
																					objReached.Add(_ray.collider.gameObject.transform.parent.gameObject);
																				}else{
																					objReached.Add(_ray.collider.gameObject);
																				}
																			}
																		}else{
																			if(_ray.collider.gameObject.transform.parent){
				...
				IEnumerator checkOnEnterOnExitEvents(){
						...
						if(notifyGameObjectsReached == true)
						{
								...
								// OnExit
								ToChange.Clear();
								//foreach( GameObject _currentObject in listObjectsInFOV)
								for (int i = 0; i < listObjectsInFOV.Count; i++)
								{
										...
										{
												if(_currentObject.GetHashCode() == tmpGO.GetHashCode())
												{
														found = true;
														break;
												}
								...
								foreach( GameObject tmpGO in objReached)
								{
										...
										foreach( GameObject _currentObject in listObjectsInFOV)
										{
												if( tmpGO.GetHashCode() == _currentObject.GetHashCode())
												{
														found = true;
														// Enhanced Persistent check if is in the sight
														if(objectsWithinSight){
															// CHECK BEFORE BREAK
															DDLEvent_InsideFOV.Invoke(tmpGO);
														}
										...
										if(found == false){
												// If GO is out of FOV
												listObjectsInFOV.Add(tmpGO);
												
												if(DDLEvent_OnEnterFOV != null)
													DDLEvent_OnEnterFOV.Invoke(tmpGO);
												
												if(OnEnterFieldOfView != null){
													OnEnterFieldOfView(tmpGO, this);
												}
